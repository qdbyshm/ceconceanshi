...
@interface LXF_OpenUDID (Private)
...
//
+ (NSString*) value {
    return [LXF_OpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [LXF_OpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            openUDID = [LXF_OpenUDID _generateFreshOpenUDID];// BY LXF   首次使用UDID，去生成获取。
